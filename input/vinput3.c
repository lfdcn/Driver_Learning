...
void callback(struct timer_list *timer)
{
    ...
    if (count_event % 3)
    {
        input_report_abs(vinput, ABS_MT_POSITION_X, 1000);
        input_report_abs(vinput, ABS_MT_POSITION_Y, 1700);
        input_sync(vinput);
        printk(KERN_WARNING "Sendevent!sendevent0\n");
    }
    else
    {
        input_report_abs(vinput, ABS_MT_POSITION_X, 200);
        input_report_abs(vinput, ABS_MT_POSITION_Y, 1000);
        input_sync(vinput);
...
static int __init vintput_init(void)
{
    ...
    input_set_abs_params(vinput, ABS_Y, 0, 2338, 0, 0);
